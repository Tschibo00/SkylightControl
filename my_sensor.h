...
#define LIGHT_ARRAY_SIZE 100
#define FILTER_WEIGHT 0.003f
...
long nextLightSensorReading = 0L;     // light sensor values
float lightLevelFloat;  // corrected values (0.0-1.0)
float medLightLevel = 0.f;

/*
 * Reads analog value of light sensor (every random time (100-1000ms) to avoid lag)
 */
float getLightSensorReading() {
  if (millis() > nextLightSensorReading) {
    float curLightLevel = analogRead(PIN_LIGHT_SENSOR);
    medLightLevel = (1.f-FILTER_WEIGHT) * medLightLevel + FILTER_WEIGHT * curLightLevel;
    ...
    nextLightSensorReading = millis() + 100 + random(900);
  }
