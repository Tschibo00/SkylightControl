  ...
  server.on("/configset", HTTP_GET, [](){
    ...
    if (server.args()) {
      for (int i = 0; i < server.args(); i++){
        ...
        if (server.argName(i)=="FONT") FONT=strtol(server.arg(i).c_str(),0,0);
      }
